...
const STORE_MATCHERS: Array<{ patterns: string[]; store: Retailer }> = [
  { patterns: ["cargills", "cargillsonline", "cargill"], store: "cargills" },
  { patterns: ["keells", "keellssuper", "keell"], store: "keells" },
  { patterns: ["spar2u", "spar 2u", "spar 2 u"], store: "spar2u" },
  { patterns: ["glowmark", "glomark", "glow mark"], store: "glowmark" },
];
...
function resolveStoreId(value: string | undefined | null): Retailer | null {
  ...
  for (const matcher of STORE_ID_MATCHERS) {
    if (matcher.ids.includes(normalizedValue)) return matcher.store;
  }
  return null;
}
...
export function mapApiProductToProduct(product: ApiProduct): Product {
  ...
  for (const price of product.prices) {
    ...
    if (store) prices[store] = price.price;
  }
